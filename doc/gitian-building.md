...
*Setup instructions for a Gitian build of MillenniumClub Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MillenniumClub
...
- [Building MillenniumClub Core](#building-millenniumclub-core)
...
Clone the git repositories for MillenniumClub Core and Gitian.
...
in the MillenniumClub Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building MillenniumClub Core
----------------

To build MillenniumClub Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the MillenniumClub Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/millenniumclub/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/millenniumclubpay/millenniumclub
...
For example:
```bash
URL=https://github.com/crowning-/millenniumclub.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit millenniumclub=${COMMIT} --url millenniumclub=${URL} ../millenniumclub/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit millenniumclub=${COMMIT} --url millenniumclub=${URL} ../millenniumclub/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit millenniumclub=${COMMIT} --url millenniumclub=${URL} ../millenniumclub/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the millenniumclub git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../millenniumclub/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/millenniumclub
SIGPATH=/some/root/path/millenniumclub-detached-sigs

./bin/gbuild --url millenniumclub=${BTCPATH},signature=${SIGPATH} ../millenniumclub/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/millenniumclub-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/millenniumclub-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/millenniumclub-osx-build.assert
```
...
[millenniumclub/gitian.sigs](https://github.com/millenniumclubpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
