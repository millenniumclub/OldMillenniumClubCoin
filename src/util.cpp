...
const char * const BITCOIN_CONF_FILENAME = "millenniumclub.conf";
const char * const BITCOIN_PID_FILENAME = "millenniumclubd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "millenniumclub" is a composite category enabling all MillenniumClub-related debug output
                if(ptrCategory->count(std::string("millenniumclub"))) {
...
#else
    const char* pszModule = "millenniumclub";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MillenniumClub
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MillenniumClub
    // Mac: ~/Library/Application Support/MillenniumClub
    // Unix: ~/.millenniumclub
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MillenniumClub";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MillenniumClub";
#else
    // Unix
    return pathRet / ".millenniumclub";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty millenniumclub.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override millenniumclub.conf
